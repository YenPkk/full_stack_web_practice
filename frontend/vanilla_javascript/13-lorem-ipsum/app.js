...
form.addEventListener("submit", (event) => {
  event.preventDefault(); // 取消預設行為，預設是將資料傳給後端server
  // console.log("hello"); // 可以看有沒有prevent時，是否印出hello的差異
  let value = amount.value;
  console.log(`value type is ${typeof value}`); // all input return a string (不論你在input元素設定時的type是甚麼，這個type只限制使用者的輸入)
  value = parseInt(value); // 所以要轉成數字
  ...
  if (isNaN(value) || value <= 0 || value > 9) {
    ...
    let tempText = text.slice(0, value); // array.slice()就跟python list[:] 的概念一樣，javascript中string或array slicing都要用這樣的形式使用
